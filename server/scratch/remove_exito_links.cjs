...
function removeExitoLinks(filePath) {
  ...
  try {
    ...
    const updatedProducts = products.map(p => {
      if (p.purchaseLink === "https://www.exito.com") {
        p.purchaseLink = "";
        count++;
      }
      return p;
    ...
    console.log(`Successfully updated ${filePath}. Cleared ${count} Exito links.`);
  } catch (err) {
...
removeExitoLinks(rootPath);
removeExitoLinks(serverPath);
